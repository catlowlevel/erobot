...
export class Coindar {
    ...
    async fetchEvents(opt: { coinIds?: string[]; startDate: string; endDate?: string }) {
        ...
        if (response.status !== 200) throw new Error("Fail to fetch events!" + response.statusText);
    ...
    private async fetchCoins() {
        ...
        if (response.status !== 200)
            throw new Error("Error while trying to fetch coins, Reason : " + response.statusText);
