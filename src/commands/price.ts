...
export default class extends BaseCommand {
	...
	public override execute = async (M: Message, args: IArgs): Promise<any> => {
		...
		for (const {
			...
			const last24HourChange = getPercentageChange(price, price24Hour).toFixed(
				2
			);

			messages += "10m : ";
			messages +=
				price <= price10m ? `-${last5MinChange}%` : `${last5MinChange}%`;
			messages += "\n";

			messages += "1h : ";
			messages +=
				price <= priceHour ? `-${lastHourChange}%` : `${lastHourChange}%`;
			messages += "\n";

			messages += "4h : ";
			messages +=
				price <= price4Hour ? `-${last4HourChange}%` : `${last4HourChange}%`;
			messages += "\n";

			messages += "24h : ";
			messages +=
				price <= price24Hour ? `-${last24HourChange}%` : `${last24HourChange}%`;
