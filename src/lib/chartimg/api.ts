...
export const validIndicator = (indicator: string) => {
    return indicators.some((c) => indicator.toLowerCase().startsWith(c.toLocaleLowerCase()));
};
export const validTf = (tf: string) => {
    //[1m, 3m, 5m, 15m, 30m, 45m, 1h, 2h, 3h, 4h, 1d, 1w]
    return ["1m", "5m", "15m", "30m", "1h", "2h", "4h", "1d", "1w"].some((s) => s === tf);
};
...
export const getChartImg = async (symbol: string, tf: string, indicators?: string[]) => {
    const urlParam = new URLSearchParams({
        key: API_KEY,
        symbol,
        interval: tf,
